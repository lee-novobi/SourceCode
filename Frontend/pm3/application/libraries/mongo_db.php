...
class Mongo_db
{
	 ...
	 * Use read preference.
	 *
	 * @var PRIMARY|SECONDARY|RP_PRIMARY_PREFERRED|RP_SECONDARY_PREFERRED
	 * @access private
	 */
	 
	private $_read_preference = 'primary';
	
	private $_dbhostflag = FALSE;

	private $_retry = 3;
	private $_retry_interval = 1;
	 ...
	 * @return void
	 */
	public function __construct($params = array('default'))
	{
		$config = $params[0];
	...
	public function switch_db($database = '')
	{
		...
		{
			$this->_show_error('Unable to switch Mongo Databases: ' . $database . ' ' . $exception->getMessage(), 500);
		}
	}
	...
	*/
	...
	private function _connect()
	{
		...
		if ($this->_dbhostflag === FALSE && !isset($options['db']))
		{
			$options['db'] = $this->_dbname;
		}
		...
		do
		{
			try
			{
				...
				if ($this->_replica_set !== FALSE && $this->_read_preference !== FALSE)
				{
					$this->_connection->setReadPreference($this->_read_preference, array());
				}
			...
			catch (MongoConnectionException $exception)
			{
				if($try > $this->_retry)
				{
					...
					{
						$this->_show_error(sprintf('Unable to connect to MongoDB (Try: %s): %s', $try, $this->_connection_string), 500);
					}
					else
					{
						$this->_show_error(sprintf('Unable to connect to MongoDB (Try: %s): %s %s', $try, $this->_connection_string, $exception->getMessage()), 500);
					}
				}
				else
				{
					sleep($this->_retry_interval);
				}
			}
		}
	...
	private function _connection_string()
	{
		...
		$this->_retry          = (int)trim($this->_config_data['mongo_connect_retry']);
		$this->_retry_interval = (int)trim($this->_config_data['mongo_retry_interval']);
		$this->_read_preference = $this->_config_data['mongo_read_preference'];

		$this->_dbhostflag = (bool) $this->_config_data['mongo_host_db_flag'];
		...
		{
			$this->_show_error('The username and password must be set to connect to MongoDB', 500);
			//$connection_string .= $this->_user . ':' . $this->_pass . '@';
		}

		$connection_string .= $this->_host;

		if ($this->_dbhostflag === TRUE)
		{
