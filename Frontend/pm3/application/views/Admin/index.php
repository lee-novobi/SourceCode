...
function LoadRolesPermission(){
	...
	$("#divRolesPermission").load('<?php echo $base_url ?>pm_permission/ajax_load_roles_permission?gid=' + strUserGroupId, function(){
		FreezHeader();
	});
}
function onRolesPermissionSubmit(){
	...
	$("#divRolesPermission input[type=checkbox]").each(function(index){
		...
		var strPnType = $(this).attr("pn_type");
		var nValue = this.checked?1:0;

		arrData.push({role_id: strRoleId, pn_id:strPnId, value: nValue, pn_type: strPnType})
	});
	$.ajax({
		type: 'POST',
	    url: "<?php echo $base_url ?>pm_permission/save_list",
	    data: {user_group_id: strUserGroupId, data: JSON.stringify(arrData)},
	    dataType: "json",
	    success: function(respond){
	    	var url = "<?php echo $base_url ?>pm_permission?gid=" + strUserGroupId;
	    	window.location = url;
	    }
	});
