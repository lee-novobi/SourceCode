<table id="table-roles-permission" cellpadding="0" cellspacing="0" width="100%" class="list">
	...
	<tr row-type="item" class="<?php echo (($row_alternate=!$row_alternate)? 'odd' : 'even') ?>">
		...
		<?php foreach($arrRole as $oRole){ ?>
  		<td class="t-center"><input value="1" type="checkbox" role_id="<?php echo $oRole['_id'] ?>" pn_id="<?php echo $oPn['_id'] ?>" <?php echo (@empty($arrSelectedGroupPermission[$oRole['_id'].$oPn['_id']])?'':'checked="checked"') ?>></td>
  		<?php } ?>
	</tr>
	<?php } ?>
	</tbody>
