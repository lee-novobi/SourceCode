...
<input type="hidden" name="division_id" value="<?php echo $oCI['division_id'];?>" />
        ...
        <td valign="top" col="1"><table width="100%" cellspacing="0" cellpadding="0" class="detail-zebra td-bordered" group="Basic Information">
            <tbody>
              ...
              <tr class="odd">
                 ...
                 <td>
                	<input id="iptFACode" field="FA Code" type="text" value="<?php echo (isset($oCI['fa_code']) ? $oCI['fa_code'] : "") ?>" name="fa_code" />
                 </td>
              </tr>
              <tr class="even">
               	 <td class="wp120" align="right"><label class="bold">FA Alias</label></td>
                 <td>
                	<input id="iptFAAlias" field="FA Alias" type="text" value="<?php echo (isset($oCI['fa_alias']) ? $oCI['fa_alias'] : "") ?>" name="fa_alias" />
                 </td>
              </tr>
              <tr class="odd">
                <td class="wp120" align="right"><label class="bold">Status</label></td>
                <td>
                	<input disabled="disabled" type="radio" name="status" value="<?php echo VALUE_DEPARTMENT_STATUS_ACTIVE ?>" <?php if (isset($oCI['status']) && $oCI['status'] == VALUE_DEPARTMENT_STATUS_ACTIVE) {?>checked="checked"<?php } ?> /><?php echo STR_DEPARTMENT_STATUS_ACTIVE ?>
                    <input disabled="disabled" type="radio" name="status" value="<?php echo VALUE_DEPARTMENT_STATUS_INACTIVE ?>" <?php if (isset($oCI['status']) && $oCI['status'] == VALUE_DEPARTMENT_STATUS_INACTIVE) {?>checked="checked"<?php } ?> /><?php echo STR_DEPARTMENT_STATUS_INACTIVE ?>
                </td>
              </tr>
          ...
          <table width="100%" cellspacing="0" cellpadding="0" class="detail-zebra td-bordered" group="Operation Information">
            <tbody>
             ...
             <tr class="even">
                ...
                <td>
                	<input type="text" value="<?php echo (isset($oCI['division_alias']) ? $oCI['division_alias'] : "") ?>" name="division_alias" readonly="readonly" />
                </td>
              </tr>
            </tbody>
          </table>
